...
const AdminUserComp = () => {
    ...
    const [userCount, setUserCount] = useState({all: 0, active: 0, banned: 0, deleted: 0});
    ...
    const fetchUserCounts = async () => {
        try {
            const response = await fetch(`${process.env.REACT_APP_API_URL}/admin/users/count`,
                {
                    method: 'GET',
                    headers: {
    ...
    const fetchUsers = async () => {
        try {
            const response = await fetch(`${process.env.REACT_APP_API_URL}/admin/users?page=${currentPage}&status=${status}&search=${searchQuery}`,
                {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',},
  ...
  return (
    <Container fluid className="p-4">
      ...
      <Row className="mb-4">
        <Col>
          <Card className="text-center status-card" onClick={() => handleStatusClick("")}>
            <Card.Body>
        ...
        <Col>
          <Card className="text-center status-card" onClick={() => handleStatusClick("active")}>
            <Card.Body>
        ...
        <Col>
          <Card className="text-center status-card" onClick={() => handleStatusClick("banned")}>
            <Card.Body>
          ...
          {users.map((user, index) => (
            <tr key={index}>
              ...
              <td>
                ...
                {...(user.status === 'deleted' ? { disabled: true } : { active: true })}
                >
                    Action</Button>
              </td>
            </tr>
        ...
        >
            ...
            <Modal.Footer>
                {selectedUser?.status === 'banned' ? (
                    <>                    
                        <Button variant="primary" onClick={handleActivateAction} >Reactivate user</Button>
                        <Button variant="primary" onClick={handleDeleteAction} >Delete user</Button>
                        <Button variant="secondary" onClick={handleClose}>Close</Button>
                    </>
                ) : (
                    <>
                        <Button variant="primary" onClick={handleBanAction}>Ban user</Button>
                        <Button variant="secondary" onClick={handleClose}>Close</Button>
                    </>
                )}
            </Modal.Footer>
